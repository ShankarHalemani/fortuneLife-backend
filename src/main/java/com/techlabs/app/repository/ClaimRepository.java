...
public interface ClaimRepository extends JpaRepository<Claim, Long> {
	...
	Long countByClaimStatus(@Param("status") String status);

	@Query("SELECT COALESCE(SUM(c.claimAmount), 0) FROM Claim c WHERE c.claimStatus = 'APPROVED'")
	Double getTotalApprovedClaimAmount();

	@Query("SELECT FUNCTION('to_char', c.date, 'YYYY-MM') AS month, COALESCE(SUM(c.claimAmount), 0) AS total " +
			"FROM Claim c WHERE c.claimStatus = 'APPROVED' GROUP BY FUNCTION('to_char', c.date, 'YYYY-MM') " +
			"ORDER BY month DESC")
	List<Object[]> getMonthlyClaimExpenses(Pageable pageable);
}
