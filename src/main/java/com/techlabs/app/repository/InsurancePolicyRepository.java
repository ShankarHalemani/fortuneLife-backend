...
public interface InsurancePolicyRepository extends JpaRepository<InsurancePolicy, String> {
        ...
        @Query("SELECT COUNT(p) FROM InsurancePolicy p WHERE p.policyStatus = :status")
        Long countByPolicyStatus(@Param("status") String status);

        @Query("SELECT p FROM InsurancePolicy p JOIN p.customer c JOIN c.user u JOIN p.insuranceScheme s WHERE " +
                        "p.id LIKE CONCAT('%', :keyword, '%') " +
                        "OR LOWER(CONCAT(u.firstName, ' ', u.lastName)) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
                        "OR LOWER(s.schemeName) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
                        "OR LOWER(p.policyStatus) LIKE LOWER(CONCAT('%', :keyword, '%'))")
        List<InsurancePolicy> searchByKeyword(@Param("keyword") String keyword, Pageable pageable);
}
